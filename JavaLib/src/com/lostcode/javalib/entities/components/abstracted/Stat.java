package com.lostcode.javalib.entities.components.abstracted;
...
/**
 * Base class for Components such as a health bar, magic meter, or any other
 * stat with a max value.
 * 
 * @author Natman64
 * 
 */
public abstract class Stat implements Component {
	...
	public void setCurrentValue(double current) {
		...
		if (current < this.current)
			onDrain(this.current - current); // Call drain event
		if (current > this.current)
			onFill(current - this.current); // Call fill event
	 ...
	 * 
	 * @param world
	 *            The EntityWorld. Needed for attaching a RegenerationProcess.
	 * @param rate
	 *            The rate of regeneration, in seconds.
	 */
	public void setRegenerationRate(EntityWorld world, double rate) {
		if (rate == 0) {
			if (regenerationProcess != null)
				regenerationProcess.end(ProcessState.ABORTED);
		} else {
			if (regenerationProcess == null)
				regenerationProcess = new RegenerationProcess(this, rate);
			else
				regenerationProcess.setRate(rate);
	 ...
	 * 
	 * @param max
	 *            The desired value. If negative, nothing will happen.
	 */
