...
public class View extends Sensor {
	 ...
	 * @param viewRange
	 *            The range of this View.
	 * @param fov
	 *            The field of view.
	 * @param vertices
	 *            The number of vertices in the fixture. Must be above 3, but
	...
	/**
	 * Sets the range of this sensor's view, in meters. Takes effect only after
	...
	/**
	 * Sets this sensor's field of view, from 0 to 1. Takes effect only after
	 * {@link #refresh()} is called.
	 * 
	 * @param fov
	...
	/**
	 * Sets this sensor's offset from the angle of its body, in degrees. Takes
	 * effect only after {@link #refresh()} is called.
	 * 
	 * @param degrees
	...
	/**
	 * Sets the position of the View sensor relative to its parent body's
	 * position.
	 * 
	 * @param position
	 */
	...
	public void refresh() {
		...
		for (int i = 1; i < vertices; i++) {
			float angle = start - ((float) i / (vertices))
					* (float) Math.toRadians(degrees);

			verts[i] = new Vector2(viewRange * (float) Math.cos(angle),
