...
/**
 * Component wrapper for multiple Components of the same type. For instance, an
 * entity requiring multiple sprites could contain a MultiComponent<Sprite>.
 * MultiComponents contain a LibGDX array of contained components, but also a
 * single base Component. Whenever information is needed from the
 * MultiComponent, and each Component's information is different, the accessor
 * from base will be used.
 * 
 * @author Natman64
 * @created Aug 21, 2013
 * @param <T>
 *            The type of Component that will be stored in the MultiComponent.
 */
public class MultiComponent<T extends Component> implements Component {
	...
	public void reorder(Array<Integer> order) {
		...
		if (order.size != children.size)
			throw new IllegalArgumentException(
					"The order array's size does not match the size of the children array.");
		...
		if (duplicateSet.size() != order.size)
			throw new IllegalArgumentException(
					"The order array contained duplicate indices.");
	...
	/**
	 * @param index
	 *            The index of one of this MultiComponent's children.
